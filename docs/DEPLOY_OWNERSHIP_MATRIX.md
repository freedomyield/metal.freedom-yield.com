...
| `public/api/peers-history-index.json` | `scripts/peer-validators.sh` (inline `python3 -c`, not `peer-analytics.py` — that script writes the peers-changes/gini/fee-market family only; corrected 2026-08-06, review round 1) | validator host | NO | YES | NO | YES (pre-existing) | Re-derive on validator host. |
...
| `public/api/build.json` | GitHub Actions `deploy.yml`, step "Write build.json (deploy manifest)" (added 2026-08-06 — `.gitignore` and `validate.yml` had assumed a producer since before this file existed; see the "no detached `.sig`" note above for the sibling gap this was found alongside) | GitHub Actions runner, per deploy run | NO (`.gitignore`) | NO (git-deploy owned; written into the runner's local `public/` checkout before both rsync legs, so it ships with the rest of `public/`) | YES (produced by the deploy workflow itself) | N/A (regenerated by the deploy that would otherwise re-add it; not a host-pushed feed) | Not re-derivable after the fact (it names the commit/run that produced it) — the next successful deploy overwrites it with current values; a stale value simply means the last deploy is what's live. |
...
A fourth shape exists but is rare: **CI-runner-generated, not committed,
not host-pushed** (`public/api/build.json` is the only current example) —
the deploy workflow itself writes the file into the runner's local
`public/` checkout before either rsync leg runs, so it needs neither a
Git commit nor a `feed-excludes.txt` entry; it simply rides the same
rsync as every other file under `public/` and is regenerated fresh every
deploy. Document the exact workflow step that writes it (not "n/a") so
this shape isn't mistaken for #1's "already committed" case.
