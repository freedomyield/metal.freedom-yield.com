...
| `public/api/cycles-history.json` | `scripts/operator-local/gen-identity.sh` (= **retired** 2-branch DAG snapshot; the inscribed root is now `anchor-source.json .dag_root_computed`, but `gen-identity.sh` still emits this file pending design-debt ① retirement) | operator Mac | YES | NO | YES | NO | From Git; vestigial — not the on-chain anchor. |
