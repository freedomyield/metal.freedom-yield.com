...
A cron whose invoked script never notifies, never pushes, and never touches `/var/lib/freedom-yield` does not need the line — e.g. the `node-info` / `server-status` crons only overwrite a `public/api/*.json` file directly and are out of scope for this rule. `scripts/check-cron-file.sh` Rule 6 enforces this: it fails a candidate cron whose command references a known side-effecting script (or one that sources `scripts/lib/side-effects.sh`) without a `FY_LIVE=1` line, and passes everything else without requiring the line.
