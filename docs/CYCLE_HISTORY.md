...
For each cycle in `uptime-cycles.json.cycles[]`, sorted by `cycle_n`, it emits a JSON line whose `incidents_in_cycle_count` and `incidents_in_cycle_ids` come from selecting incidents whose `detectionDate` falls in the half-open interval `[start_iso, end_iso)`, **compared by date part only**.

The date-part truncation matters: `detectionDate` is a date (`2026-06-24`) while `start_iso`/`end_iso` are timestamps (`2026-05-19T07:10:14Z`). Compared as raw strings, the shorter date always sorts before the same-day timestamp, so an incident detected on a cycle's own start date fell into the *previous* cycle — and one detected on the first cycle's start date (`2026-05-19`, the validator's first day) belonged to no cycle at all and disappeared from the feed. With both sides truncated, a boundary-date incident belongs to **the cycle that starts that day**, windows stay contiguous, and every incident lands in exactly one cycle. The generator enforces that property with a conservation check (exit 4) rather than trusting it, and `tests/gen-cycle-history/test-incident-attribution.sh` covers both boundaries. (`detectionDate` is the field name `incidents.schema.v1.json` declares; the generator read a non-existent `date` field until 2026-08-06, which silently pinned both fields to `0` / `[]` — see the comment block in `scripts/gen-cycle-history.sh`.)
