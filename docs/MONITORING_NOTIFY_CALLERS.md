...
## Addendum (2026-09-07) — `reward-tracker.sh` caller + the `[Reward]` splice in `daily-status.sh`

Sections 1–7 above are a snapshot at `316e8f1` and are left as written. This addendum records one caller that did not exist at that snapshot and one change to caller #4's body, both live since 2026-09-07 (`/etc/cron.d/metal-reward-tracker`, daily 22:35 UTC, installed by `scripts/install-reward-tracker-cron.sh`). Line numbers below were measured at `2d4c049`, not at `316e8f1`.

**Why the §1 inventory grep does not find it.** `scripts/reward-tracker.sh` never spells `notify.sh` or `"$NOTIFY"`: it sources `scripts/lib/side-effects.sh` (line 324) and calls that library's `fyd_notify` wrapper — `fyd_notify()` starts at `side-effects.sh:386`; its delegate-resolution + execution block (`notify_bin` resolved to `notify.sh` by default, then `bash "$notify_bin" …`) is `side-effects.sh:433-478`. The §1 patterns return nothing for this file (measured: `grep -n 'bash.*notify\.sh\|"\$NOTIFY"' scripts/reward-tracker.sh` → exit 1). The post-C3 equivalent of the §1 inventory is `grep -rn 'fyd_notify' --include='*.sh' scripts/`; that is the grep that lists this caller (and the other migrated ones, which are out of this addendum's scope).

| # | Caller (file:line) | Form | `set` flags in effect at call site | Position in script | Exit-code handling | Behaviour under non-zero from notify.sh | Risk under exit-code change |
|---|---|---|---|---|---|---|---|
| 6 | `scripts/reward-tracker.sh:995` (= matured cycle, zero-reward branch) and `scripts/reward-tracker.sh:1015` (= matured cycle, non-zero branch, `--tags=tada`) — exactly ONE of the two fires per matured cycle | `fyd_notify [--tags=tada] high "…" "$BODY" >/dev/null` — via `scripts/lib/side-effects.sh`, not a direct `bash "$NOTIFY"`; no `--strict`, so notify.sh runs under today's default contract (§3), and `fyd_notify` returns notify.sh's rc unchanged | `set -uo pipefail` (line 314; no -e) | mid-script, inside the maturity branch only — AFTER the ledger append (line 918) and `write_state` (line 924), BEFORE the digest regeneration (`compute_and_write_digest`, line 1147). Steady state (bootstrap / same cycle still in flight / maturity deferred) and `--backfill` never reach either call: zero pushes | No `\|\| true`, but the rc is discarded (no -e, the statement is not in a condition) | Ignored; the run continues to the digest. Because the tracked state already advanced before the push, a failed push is NOT retried on a later run — one shot, best-effort. The ledger row and the next morning's `[Reward]` block are the durable record either way | **None** today (rc never read). If this call is ever moved onto `--strict` + retry, note the order append → state → push: a K-3-style "do not advance state until delivered" pattern (§6) would need the push to move ahead of `write_state`, which is not how it is wired now |
| 4 (body change, same call) | `scripts/daily-status.sh:318` (= the digest push; was line 228 / `bash "$NOTIFY"` at `316e8f1`, now `fyd_notify "$PRIO" "${TITLE_PREFIX} ${DATE_JP}" "$BODY"`) | unchanged notify route — the `[Reward]` splice (lines 260–274) only adds text to `$BODY` (inserted at line 314, before `[Evidence health]`) | `set -uo pipefail` (line 18); `set +e` at 223 / `set -e` at 226 around the evidence subprocess → **`-e` active at line 318**, as in §2.1 | **Last statement** of the script (unchanged) | unchanged from #4 | unchanged from #4 | **Low** — unchanged from #4. The splice adds no notify call and no failure mode: morning slot only; reads `${FY_STATE_DIR}/reward-digest-line.txt` (`fyd_state_dir` with no role, line 263 — the same dir `reward-tracker.sh` writes); a missing or blank-only file yields no block and no warning (`-s` test + blank-line filter, lines 265–267); priority escalation (lines 281–287) does not look at it |

Two properties worth stating once, since they are what keeps this addition quiet:
