...
- **失敗コードの意味**:
  ...
  | **1**(step 9/10 で `ERROR (3): gate 1 — tx_id … not resolvable`) | **Hyperion の索引遅延。broadcast は成功している。** 2026-09-01 の稽古で実測: `BROADCAST OK` 直後は v2/v1 とも未索引、約 1 分後に両方で解決。`gen-anchor-receipt.sh` は 1 回しか問い合わせず待機もリトライもしない(`:159-197`) | **③ を再実行しない**(同じ cycle の二重 broadcast になり、gate は止めない)。約 1 分待って **AI が step 9 (`gen-anchor-receipt.sh`) だけ**を再実行する — step 8 の入力 JSON は `~/.fya-testnet-dryrun-log.json` から同じ jq で再構成できる |
