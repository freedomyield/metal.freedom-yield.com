...
2. **The reachable strategic surface is:** (a) ~~Metallicus non-banking subnet selection~~ → *(revised 2026-08-17)* **selection into a public A-Chain subnet's validator set**, run on PulseVM — the same path, named concretely now that the venue and its selection mechanism are documented; see reachable-set item 1, (b) organic delegator flow from pseudonymous evaluators, (c) tooling / data producer positioning inside the Metallicus orbit. All three tolerate pseudonymous operators with verifiable technical substance.
