...
| `live_artifacts` | object | map of slugs to artefacts whose runtime URL is live today — see below |
| `in_preparation_artifacts` | object | map of slugs to artefacts that are not yet live but whose schema is published — see below (currently empty; every artefact this manifest tracks is live) |
...
The `live_artifacts` object surfaces artefacts whose **runtime URL is live and served from this site**. It exists so an automated reviewer can discover the full audit surface without having to read prose anywhere. Each entry carries `url` + `schema_url` at minimum, plus whichever of `signature_url` / `pubkey_url` / `formal_schema_url` / `format_guide_url` / `operator_guide_url` / `cross_reference_url` apply to that artefact.
...
- `anchor_receipt` — the A-chain inscribe receipt (`/api/anchor-receipt.json`, v2 schema), live since the cycle 2 → 3 transition (2026-07-04). `formal_schema_url` points at `anchor-receipt.schema.v2.json`, not v1 — the live file is generated by `scripts/gen-anchor-receipt.sh`, which validates against v2.
- `anchor_history` — the append-only anchor log (`/api/anchor-history.jsonl`, v2 schema), live since the same 2026-07-04 transition. `formal_schema_url` likewise points at `anchor-history.schema.v2.json` (matches `scripts/append-anchor-history.sh`'s actual validation target); `schema_url` currently still points at the v1-shaped `anchor-history.example.jsonl` preview, since no v2-shaped example file exists yet — a known follow-up, not a live-vs-schema mismatch in the runtime file itself.
...
Currently empty — every artefact this manifest has ever tracked (`identity_manifest`, `cycle_history_jsonl`, `anchor_receipt`, `anchor_history`) is live. The mechanism stays documented here for the next artefact that publishes its schema ahead of going live.

When an in-preparation artefact goes live, its entry moves out of `in_preparation_artifacts` and into `live_artifacts` (or, for page-like artefacts, into `public_pages`). The `_comment` field is updated to call out the change so a diffing consumer sees the migration explicitly.
...
- `/api/cycle-history.jsonl` — live append-only cycle audit packet documented at `docs/CYCLE_HISTORY.md`.
- `/api/identity.json` — signed operator identity manifest (= live since Phase 5, 2026-06-22) documented at `docs/IDENTITY_VERIFICATION.md`. Companion artifacts: `/api/identity.json.sig`, `/api/anchor-source.json` (v2 3-branch DAG source), `/api/anchor-receipt.json` (A-chain inscribe receipt, live since 2026-07-04), `/api/anchor-history.jsonl` (append-only anchor log, live since 2026-07-04), `/api/identity-history.jsonl`, `/.well-known/operator-identity.pub`.
