...
run_json "git commit --no-verify -> block"         1 "$(jq -nc --arg c 'git commit -m x --no-verify' '{tool_name:"Bash",tool_input:{command:$c}}')"
run_json "git push -n -> block"                    1 "$(jq -nc --arg c 'git push origin main -n' '{tool_name:"Bash",tool_input:{command:$c}}')"
...
echo "== Write/Edit =="
run_json "write public IP -> tracked README block" 1 "$(jq -nc --arg fp "$REPO_ROOT/README.md" --arg c "host $PUB_IP" '{tool_name:"Write",tool_input:{file_path:$fp,content:$c}}')"
run_json "write phone -> tracked README block"     1 "$(jq -nc --arg fp "$REPO_ROOT/README.md" --arg c "tel $FAKE_PHONE" '{tool_name:"Write",tool_input:{file_path:$fp,content:$c}}')"
...
run_json "edit forbidden word -> tracked block"    1 "$(jq -nc --arg fp "$REPO_ROOT/README.md" --arg c "$TEST_WORD" '{tool_name:"Edit",tool_input:{file_path:$fp,new_string:$c}}')"
run_json "multiedit public IP -> block"            1 "$(jq -nc --arg fp "$REPO_ROOT/README.md" --arg c "host $PUB_IP" '{tool_name:"MultiEdit",tool_input:{file_path:$fp,edits:[{old_string:"a",new_string:"b"},{old_string:"c",new_string:$c}]}}')"
