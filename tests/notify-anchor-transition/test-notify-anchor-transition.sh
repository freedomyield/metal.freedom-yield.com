...
EOF
chmod +x "$STUB8"
ANCHOR_NOTIFY="$STUB8" NOTIFY_RETRY_SLEEP=0 bash "$DRIVER" --event-type=cyclestart || rc=$?
...
EOF
chmod +x "$STUB9"
ANCHOR_NOTIFY="$STUB9" NOTIFY_RETRY_SLEEP=0 bash "$DRIVER" --event-type=cycleend || rc=$?
...
EOF
chmod +x "$STUB10"
ANCHOR_NOTIFY="$STUB10" NOTIFY_RETRY_SLEEP=0 bash "$DRIVER" --event-type=cyclestart || rc=$?
