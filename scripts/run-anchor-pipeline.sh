...
#
# AFTER the pipeline — publication of the two FLAT files (deliberately NOT
# done here): the run's canonical receipt + history artifacts reach their
# public URLs from the validator host via
...
# it; see docs/DEPLOY_OWNERSHIP_MATRIX.md.) Pushing stays outside this
...
#
# The R18 per-anchor archives (archive/anchor-source-<dag_root>.json,
# archive/anchor-receipt-<tx_id>.json — what anchor-history.jsonl's
# archived_source_path / archived_receipt_path advertise) are DIFFERENT:
# step 4 (append-anchor-history.sh) pushes those two itself, automatically,
# right after its append succeeds (2026-08-06) — no separate manual step
# needed for them. Before that, this was a third manual push the operator
# had to remember (and once didn't, 2026-08-04), leaving those two URLs
# 404ing. The subdirectory push shapes were added 2026-08-05 and require the
# matching web-host wrapper allowlist (scripts/install-xserver-subdir-
# allowlist.sh). Same non-fatal posture as above: a publish failure inside
# append-anchor-history.sh is reported loudly but never fails the append.
#
