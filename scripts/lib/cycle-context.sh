...
daily|-|check-identity-pins.sh|6|NEW kind-gate break: a pin exists over a kind=stream publication that is not acknowledged in deploy/publication.json's known_kind_violations, over a publication with no row in deploy/publication.json at all, or over a row whose kind is none of stream/static/record. Checked before exit 3/4, so a run with both a kind-gate break and a mismatch exits 6, not 3 - but (2026-08-17, --mode=live) the mismatch is NOT silently dropped: its first line is folded into the SAME ntfy push text and dedup signature as the kind-gate alert (scripts/check-identity-pins.sh's exit-6 alert block, where MISMATCH_FOLD_MSG is composed - :856-858 as measured 2026-08-17; cited by name because that line number has drifted before), so the operator still learns about it from the one push this run sends.|Acknowledge a still-open kind=stream case in deploy/publication.json's known_kind_violations, add a missing publications[] row, or re-issue the manifest (gen-identity.sh never pins kind=stream) so the pin is simply gone. If the alert also folded in a mismatch's first line, that mismatch still needs its own remediation (see exit 3's row) once the kind-gate break is cleared - fixing the kind-gate acknowledgment alone does not resolve it.
