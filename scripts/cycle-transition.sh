...
fyct__unit_commands() {
	...
	7c)
		...
		fyct__wrap "  " "Its stdout is additionally saved to /tmp/fya-mainnet-sign-output.json, which is unit 7.5's input. THIS PLAN PINS THAT PATH WITH AN EXPLICIT --output= RATHER THAN LETTING IT DEFAULT. Left to the default, the directory is whatever FY_SIGN_OUTPUT_DIR says (it exists so tests can redirect it), and an exported value still live in this terminal would send the fragment somewhere else while unit 7.5 goes on scp'ing the literal /tmp path — shipping the PREVIOUS cycle's fragment, which the implementation itself warns is a real file on the operator's Mac and not disposable scratch. Unit 8's 7-gate would catch the mismatch, but only after the irreversible broadcast."
		fyct__wrap "  " "Unit 7b prints this same command WITHOUT the --output= argument. If you paste 7b's form instead of this one — which is otherwise the right thing to do, since it fills in both gate args — then confirm FY_SIGN_OUTPUT_DIR is unset in that terminal first, or add the --output= from this line to it."
		if [ "$TTX_KNOWN" = "1" ]; then
...
fyct_print_plan() {
	...
	fyct__wrap "  " "There is also ONE canon-only checkpoint that is not an execution unit and has no id in the table below: CHECKPOINT 2.5, printed at the end of unit 2. Read it before unit 3. On a cycle where it applies, skipping it is the one thing today that cannot be corrected afterwards."
	...
	fyct__wrap "  " "4. Unit 4b is a set of hand edits. Only its verification and commit are commands. Two of its five bullets apply only when CHECKPOINT 2.5 ran, and one of those two is not enforced by any exit code."
