...
# six-line (three in the fallback) phone-width morning-digest block
# daily-status.sh's morning slot includes.
#
...
echo "digest:    /var/lib/freedom-yield/reward-digest-line.txt (six-line phone-width block, three in the fallback; regenerated every run; consumed by daily-status.sh's morning slot)"
