...
| `scripts/advance-host-checkout.sh` | Daily self-heal: FF-only advance the validator-host git checkout to `origin/main` (refuses + alerts high if the host is ever ahead — a host must never author; discards only `public/` deploy cache-bust dirt before pulling). Primary defense against host `HEAD` drift; see [`docs/HOST_CHECKOUT_AUTO_ADVANCE.md`](docs/HOST_CHECKOUT_AUTO_ADVANCE.md). | cron (daily, 04:45 UTC) |
