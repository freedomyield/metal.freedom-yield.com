...
| `scripts/check-identity-pins.sh` | Detect when the artifact `sha256` pins inside the operator-SIGNED `public/api/identity.json` stop describing the real bytes. `--mode=repo` (CI gate, wired into `validate.yml` + `ci-main.yml`) compares against the repo's own files; `--mode=live` (daily cron) compares against the served URLs. Classifies each pin from `deploy/feed-excludes.txt`: a **push-owned feed** pin (evidence / validator / uptime-cycles) drifts by construction and is only recorded, a **git-tracked** pin can only break because a commit broke it, so it alerts and fails. `deploy/identity-pin-baseline.json` acknowledges the pins already broken on 2026-08-06, scoped to the exact (pinned, observed) pair so any further edit is still red. Alert-only; the repair is operator-manual because CI has no access to the signing key. | daily 01:00 UTC (live) + every CI run (repo) |
